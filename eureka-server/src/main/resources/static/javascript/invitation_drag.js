 ...
 */
	...
	var stack = new Undo.Stack(),
		saveCommand = Undo.Command.extend({
			...
			execute: function(){
				$("#"+this.pageId).append(this.box);
				  iniLeftPageContent(this.pageId);
			...
			redo:function() {
				...
				$("#"+this.pageId).append(this.outerHTML);
		...
		undo: function() {
			...
			$("#"+this.pageId).append(this.boxHtml);
			$("#"+this.boxId).mousedown(boxmousedown);
			$("#"+this.boxId).dblclick(function(){
		        var type=$(this).attr("type");
		...
		//		$("#pBox").css("filter","progid:DXImageTransform.Microsoft.AlphaImageLoader(src='"+json.urlPath+"',sizingMethod='scale',scale='true')");
			$("#"+this.pageId).css({'background':this.newUrl}); $("#"+this.pageId).css({'background-size':'100%'});
			iniLeftPageContent(this.pageId);
		},
		undo: function() {
			
			$("#"+this.pageId).css({'background':this.oldUrl});  $("#"+this.pageId).css({'background-size':'100%'});
			iniLeftPageContent(this.pageId);
		},
		redo:function() {
			$("#"+this.pageId).css({'background':this.newUrl});  $("#"+this.pageId).css({'background-size':'100%'});
...
function rightBoxMove(currItem){
	...
	function u_center_body(cssnum,e){
		...
		break; 
		case 10:$("#pBox").find(".edit").css({'background':e.value}); $("#pBox").find(".edit").css({'background-size':'100%'});iniLeftPageContent($("#pBox").find(".edit").attr("id"));
		break; 
